	...
	async execute(interaction) {
		...
		for (let category in categories)
		{
			...
			if (categories[category].isCreatable) {
				...
				.setLabel(categories[category].localisation[3])
		...
		const message = await interaction.editReply({ephemeral: true, content: "Wählen Sie eine Anfragekategorie", components: rows, fetchReply: true})
		...
		collector.on('collect', async(i) => {
			const threadCreate = async (channelId, category) => {
				...
				const thread = await channel.threads.create({
					name: `de-${interaction.member.displayName}`,
				...
				if (category.hellomessage?.[3]) await thread.send({content: category.hellomessage[3]});
