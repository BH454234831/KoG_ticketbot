...
module.exports = {
	...
	async execute(interaction) {
		...
		const ingame = new ButtonBuilder()
			.setCustomId('ingame')
			.setLabel('Dans le jeu')
			.setStyle(ButtonStyle.Primary);

		const change = new ButtonBuilder()
			.setCustomId('change')
			.setLabel('Changement de pseudo')
			.setStyle(ButtonStyle.Primary);
		...
		const message = await interaction.reply({ephemeral: true, content: "Sélectionnez une catégorie de demande", components: [ticketType],fetchReply: true})
		...
		collector.on('collect', async(i) => {
			const threadCreate = async (channelId, prefix) => {
				...
				const thread = await channel.threads.create({
					name: `fr-${prefix}-${interaction.member.displayName}`,
