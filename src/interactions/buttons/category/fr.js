	...
	async execute(interaction) {
		...
		for (let category in categories)
		{
			...
			if (categories[category].isCreatable) {
				...
				.setLabel(categories[category].localisation[2])
		...
		const message = await interaction.editReply({ephemeral: true, content: "Sélectionnez une catégorie de demande", components: rows, fetchReply: true})
		...
		collector.on('collect', async(i) => {
			const threadCreate = async (channelId, category) => {
				...
				const thread = await channel.threads.create({
					name: `fr-${interaction.member.displayName}`,
				...
				if (category.hellomessage?.[2]) await thread.send({content: category.hellomessage[2]});
		...
		collector.on('end', collected => {
            if (collected.size === 0) {
                interaction.editReply({ content: "Le temps est écoulé. Vous n'avez sélectionné aucune catégorie", ephemeral: true, components: [] });
            }
        });
		}
